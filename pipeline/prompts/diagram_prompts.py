...
4. All topics must be in Chinese.
...
5. All elements must be in Chinese.
...
All questions, answers, and explanations must be in Chinese.
Do not include any additional text at the beginning or end of your response."""
...
2. **代码要求**：
    ...
    (5) **流程图方向多样化**：按以下占比选择方向
        - LR（从左到右）：50% - 最常用的水平布局
        - TD（从上到下）：30% - 常用的垂直布局  
        - RL（从右到左）：15% - 增加变化的反向水平布局
        - BT（从下到上）：5% - 少量的反向垂直布局
    (6) **流程图结构多样化**：根据数据特点选择合适的结构
        - 单一开始点，单一结束点（传统流程）
        - 多个开始点，单一结束点（汇聚流程）
        - 单一开始点，多个结束点（分流流程）
        - 多个开始点，多个结束点（并行流程）
...
3. **语法检查要求**（重要）：
    ...
    (5) **每行一个完整语句**：确保样式定义、节点定义、关系定义各占独立的行，不要混在一起
    (6) **class diagram特殊要求**：类定义必须完整独立，结束大括号后必须换行，属性名避免特殊字符
    (7) **严格禁止样式语法**：绝对不要使用classDef、class、style等样式命令，只生成纯净的图表结构
